...
class Player_Ro extends CI_Controller{
    ...
    public function prepare_player_ro(){
        ...
        $this->load->view('create_player_ro', $data);
        $this->load->view('footer_admin_area');

    }

    public function prepare_update_player_ro($entity_id){

        $data['player_details']=$this->crud_model_player->extractPlayerDetailsIDRO($entity_id);
        $data['teams']=$this->crud_model_team->extractTeamName();
        ...
        $this->loadUpdateEntityDetailsRO($data);
    }

    public function prepare_delete_player_ro($player_id){
        $data['page_title']=$this->setPageTitle('Raport preliminar stergere jucator');
        $data['player_details']=$this->crud_model_player->extractPlayerDetailsIDRO($player_id);
    ...
    public function create_player_ro(){
        ...
        if($this->crud_model_player->prevent_duplicate_player_entries_RO($this->parameters_crud['player_nickname'])){
        ...
        if($this->crud_model_player->insertPlayerDBRO($this->player_team_id,$this->parameters_crud, $this->image_path)){
    ...
    public function read_player_ro(){
        //pagination configuration for results
        $config['base_url']=base_url().'index.php/player/read_player_ro/';
        $config['total_rows']=$this->crud_model_player->countRowsPlayerRO();
        $config['per_page']=10;
        ...
        $data['player_details']=$this->crud_model_player->extractPlayerTeamNameRO($config['per_page'], $this->uri->segment(3));
        $data['entity_type']=1; //1 is a flag for PLAYER entity

        $this->pagination->initialize($config);
        $data['links']=$this->pagination->create_links();

        $this->loadEntityDetailsRO($data);
    }

    public function update_player_ro($player_id){
    ...
    public function delete_player_ro($player_id){
        $this->crud_model_player->deletePlayerRO($player_id);
        redirect('index.php/player/read_player_ro');
    }
    ...
    public function loadDeletePageReportRO($data){
        $this->load->view('header_admin_area');
        $this->load->view('delete_report_player_ro', $data);
        $this->load->view('footer_admin_area');
    }
